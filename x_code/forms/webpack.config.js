...
function makeConfig(options) {
  return {
    ...
    module: {
      loaders: [
        { test: /\.(png|jpg|gif)$/,   loader: "url?limit=50000&name=[path][name].[ext]" },
        { test: /\.json$/, loader: 'json' },
        { test: /^(?!.*\.min\.css$).*\.css$/, loader: ExtractTextPlugin.extract("style-loader", 
                                                            "css-loader?sourceMap")},
        { test: /\.svg(\?v=\d+\.\d+\.\d+)?$/,         loader: "url" },
        { test: /\.html$/,    loader: "raw" },
        { test: /^index\.html$/, loader: "file-loader?name=[path][name].[ext]" },
        { test: /\.ts$/, loader: 'typescript-simple?ignoreWarnings[]=2304', exclude: [ /test/, /node_modules/]},
